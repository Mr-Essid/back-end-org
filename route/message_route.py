...
async def get_message_of_session(session_id: str, current_user: dict = Depends(get_current_user)):
    session = await db.getCollection(Collections.SESSION).find_one({schemes.Session.ID_: ObjectId(session_id)})
    ...
    if session.D_ID != current_user.get(schemes.User.ID_DEPARTMENT) and current_user.get(
